...
//追加
#include "simulation-proposal2.h"
...
NS_LOG_COMPONENT_DEFINE ("SimulationProposal2");

NS_OBJECT_ENSURE_REGISTERED (SimulationProposal2);

TypeId
SimulationProposal2::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SimulationProposal2")
    .SetParent<Application> ()
	.SetParent<olsr::RoutingProtocol> ()
    .SetGroupName("Applications")
    .AddConstructor<SimulationProposal2> ()
    .AddAttribute ("DataRate", "The data rate in on state.",
                   DataRateValue (DataRate ("500kb/s")),
                   MakeDataRateAccessor (&SimulationProposal2::m_cbrRate),
                   MakeDataRateChecker ())
    .AddAttribute ("PacketSize", "The size of packets sent in on state",
                   UintegerValue (512),
                   MakeUintegerAccessor (&SimulationProposal2::m_pktSize),
                   MakeUintegerChecker<uint32_t> (1))
    .AddAttribute ("Remote", "The address of the destination",
                   AddressValue (),
                   MakeAddressAccessor (&SimulationProposal2::m_peer),
                   MakeAddressChecker ())
    .AddAttribute ("Local",
                   "The Address on which to bind the socket. If not set, it is generated automatically.",
                   AddressValue (),
                   MakeAddressAccessor (&SimulationProposal2::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("OnTime", "A RandomVariableStream used to pick the duration of the 'On' state.",
                   StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"),
                   MakePointerAccessor (&SimulationProposal2::m_onTime),
                   MakePointerChecker <RandomVariableStream>())
    .AddAttribute ("OffTime", "A RandomVariableStream used to pick the duration of the 'Off' state.",
                   StringValue ("ns3::ConstantRandomVariable[Constant=1.0]"),
                   MakePointerAccessor (&SimulationProposal2::m_offTime),
                   MakePointerChecker <RandomVariableStream>())
    .AddAttribute ("MaxBytes",
                   ...
                   MakeUintegerAccessor (&SimulationProposal2::m_maxBytes),
                   MakeUintegerChecker<uint64_t> ())
    .AddAttribute ("Protocol", "The type of protocol to use. This should be "
                   "a subclass of ns3::SocketFactory",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&SimulationProposal2::m_tid),
                   // This should check for SocketFactory as a parent
                   MakeTypeIdChecker ())
    .AddAttribute ("EnableSeqTsSizeHeader",
                   "Enable use of SeqTsSizeHeader for sequence number and timestamp",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SimulationProposal2::m_enableSeqTsSizeHeader),
                   MakeBooleanChecker ())
...
SimulationProposal2::SimulationProposal2 ()
  : m_socket (0),
	m_socket_local (0),
    m_connected (false),
    m_residualBits (0),
...
void
SimulationProposal2::SetMaxBytes (uint64_t maxBytes)
{
...
Ptr<Socket>
SimulationProposal2::GetSocket (void) const
{
  NS_LOG_FUNCTION (this);
...
int64_t
SimulationProposal2::AssignStreams (int64_t stream)
{
...
Ptr<Socket>
SimulationProposal2::GetListeningSocket (void) const
{
  NS_LOG_FUNCTION (this);
  return m_socket;
}
...
std::list<Ptr<Socket> >
SimulationProposal2::GetAcceptedSockets (void) const
{
  NS_LOG_FUNCTION (this);
  return m_socketList;
}
...
void
SimulationProposal2::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
// Application Methods
void SimulationProposal2::StartApplication () // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
...
//                   }
              	 ...
              	 resource_num = protocol->GetRandInstance().randnode[12];
              	 std::cout<<resource_num<<std::endl;
        		 ...
        		 m_socket->SetRecvCallback (MakeCallback (&SimulationProposal2::HandleRead, this));
               m_socket->SetAcceptCallback (
               MakeNullCallback<bool, Ptr<Socket>, const Address &> (),
               MakeCallback (&SimulationProposal2::HandleAccept, this));
               m_socket->SetCloseCallbacks (
               MakeCallback (&SimulationProposal2::HandlePeerClose, this),
               MakeCallback (&SimulationProposal2::HandlePeerError, this));
...
//                   			  m_socket_local->SetAllowBroadcast (true);
                   			  ...
                   			  m_socket_local->SetConnectCallback (
                   				MakeCallback (&SimulationProposal2::ConnectionSucceeded, this),
                   				MakeCallback (&SimulationProposal2::ConnectionFailed, this));
                   		 }
...
void SimulationProposal2::StopApplication () // Called at time specified by Stop
{
  NS_LOG_FUNCTION (this);
...
void SimulationProposal2::CancelEvents ()
{
  NS_LOG_FUNCTION (this);
...
// Event handlers
void SimulationProposal2::StartSending ()
{
  NS_LOG_FUNCTION (this);
...
void SimulationProposal2::StopSending ()
{
  NS_LOG_FUNCTION (this);
  CancelEvents ();
...
// Private helpers
void SimulationProposal2::ScheduleNextTx ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    {
      ...
      m_sendEvent = Simulator::Schedule (nextTime,
                                         &SimulationProposal2::SendPacket, this);
    }
  else
...
void SimulationProposal2::ScheduleStartEvent ()
{  // Schedules the event to start sending data (switch to the "On" state)
  NS_LOG_FUNCTION (this);

  Time offInterval = Seconds (m_offTime->GetValue ());
  NS_LOG_LOGIC ("start at " << offInterval.As (Time::S));
  m_startStopEvent = Simulator::Schedule (offInterval, &SimulationProposal2::StartSending, this);
}

void SimulationProposal2::ScheduleStopEvent ()
{  // Schedules the event to stop sending data (switch to "Off" state)
  NS_LOG_FUNCTION (this);

  Time onInterval = Seconds (m_onTime->GetValue ());
  NS_LOG_LOGIC ("stop at " << onInterval.As (Time::S));
  m_startStopEvent = Simulator::Schedule (onInterval, &SimulationProposal2::StopSending, this);
}


void SimulationProposal2::SendPacket ()
{
  NS_LOG_FUNCTION (this);
...
void SimulationProposal2::ConnectionSucceeded (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  m_connected = true;
}

void SimulationProposal2::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  NS_FATAL_ERROR ("Can't connect");
}

void SimulationProposal2::HandleRead (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Ptr<Packet> packet;
  Address from;
...
void
SimulationProposal2::PacketReceived (const Ptr<Packet> &p, const Address &from,
...
void SimulationProposal2::HandlePeerClose (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}

void SimulationProposal2::HandlePeerError (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}

void SimulationProposal2::HandleAccept (Ptr<Socket> s, const Address& from)
{
  NS_LOG_FUNCTION (this << s << from);
  s->SetRecvCallback (MakeCallback (&SimulationProposal2::HandleRead, this));
